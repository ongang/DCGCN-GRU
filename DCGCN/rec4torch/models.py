...
class GraphConvolution(RecBase):
    def __init__(self, input_dim, output_dim, linear_feature_columns, dnn_feature_columns):
        super(GraphConvolution, self).__init__(linear_feature_columns, dnn_feature_columns)
...
# 融合 Deep Crossing 和 GCN 的模型
class DeepCrossingWithGCN(RecBase):
    def __init__(self, linear_feature_columns, dnn_feature_columns, deep_crossing_model, gcn_model, adjacency_matrix, gcn_input_dim, gcn_hidden_dim, gcn_output_dim):
        super(DeepCrossingWithGCN, self).__init__(linear_feature_columns, dnn_feature_columns)
        self.deep_crossing_model = deep_crossing_model
        self.gcn_model = gcn_model
        self.adjacency_matrix = adjacency_matrix  # Store the adjacency matrix as an attribute
        self.gcn_input_dim = gcn_input_dim
        self.gcn_hidden_dim = gcn_hidden_dim
        self.gcn_output_dim = gcn_output_dim
        self.linear_feature_columns = linear_feature_columns
        self.dnn_feature_columns = dnn_feature_columns
        # Define any additional layers or operations you may need here

    def forward(self, X, adjacency_matrix):  # Modify forward method to accept adjacency_matrix
        deep_crossing_output = self.deep_crossing_model(X)
        gcn_output = self.gcn_model(adjacency_matrix)  # Pass adjacency_matrix to GCN model
        combined_output = torch.cat([deep_crossing_output, gcn_output], dim=1)
...
class NeuralCF(RecBase):
    ...
    def __init__(self, dnn_feature_columns, dnn_hidden_units=(256, 128), dnn_emd_dim=4,
        ...
        assert len(dnn_feature_columns) == 5
